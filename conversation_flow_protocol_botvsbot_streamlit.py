...
# Copy and paste your helper functions (extract_json, normalize, get_last_step, etc.) here
# ... For brevity, they are omitted in this snippet but must be included exactly as-is from your original code
...
if start_chat and not st.session_state.stop:
    ...
    while True:
        if skip_key == 0:
            ...
            matched = next((c for c in conditions if normalize_condition(c["condition"]) == normalized_matched_condition), None)
        ...
        if matched and matched["next_step"].lower() not in ["remain", "step remain"]:
            ...
            if next_step == "step 51 close the chat":
                ...
                current_step = next_step.replace("step ", "")
            matched_key = get_step_by_normalized_name(normalize(current_step), knowledge_base)
            step_data = knowledge_base.get(matched_key, {})
        else:
            st.warning("⏳ Staying in current step (condition not matched or unclear).")
        ...
        if st.session_state.stop:
            st.warning("🛑 Conversation stopped manually.")
            break
